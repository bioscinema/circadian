#' Circadian Cosine Model Fitting and Phase Inference
#'
#' @description
...
#' @param t Numeric vector of time points (same length as \code{y}).
#' @param period Numeric. Period of oscillation (hours). Default is 24.
#' @param omega Numeric. Angular frequency (\eqn{2\pi / period}). Used internally.
#' @param vcov_type Character. Covariance estimator: `"iid"`, `"HC0"`, or `"HC3"`.
#' @param object An object of class `"circadian_fit"`.
#' @param parm Character. Parameter for confidence interval: `"phi"`, `"A"`, or `"c"`.
#' @param level Confidence level for intervals. Default is 0.95.
#' @param method For intervals: `"delta"` or `"lrt"` (profile likelihood for phase).
#' @param grid_size Integer. Resolution of the grid used for LRT profile (default 1440).
#' @param L Contrast matrix for phase comparisons.
#' @param rhs Numeric vector specifying right-hand side of contrasts.
#' @param alternative Character. `"two.sided"`, `"less"`, or `"greater"`.
#' @param new_t Numeric vector of new time points for prediction.
#' @param interval `"none"`, `"confidence"`, or `"prediction"`.
#'
#' @return
#' \describe{
#'   \item{\code{fit_circadian}}{Returns a `"circadian_fit"` object containing
#'   coefficients (`c`, `A`, `phi`, `B`, `C`), standard errors, covariance matrices,
...
#'   \item{\code{predict_circadian}}{Predicted mean or interval estimates at `new_t`.}
#' }
#'
#' @examples
#' t <- seq(0, 48, by = 2)
...
confint_circadian <- function(object,
  ...
  if (parm %in% c("A","c") && method == "lrt")
    stop("LRT available only for phi; use delta for A/c.")
  ...
  if (parm == "phi") {
    ...
    if (method == "delta") {
      ...
      rss_phi <- vapply(phi_grid, function(ph) {
        Sphi <- sin(omega * t + ph)
        Xphi <- cbind(1, Sphi)
        f <- lm.fit(Xphi, y)
        sum(f$residuals^2)
      ...
      if (!any(ok)) {
        warning("LRT grid failed; fallback to delta SE.")
        se <- object$se["se_phi"]
        return(.wrap_angle(phihat + c(-1,1)*z*se))
      }
      idx <- which(ok)
      ci <- c(phi_grid[min(idx)], phi_grid[max(idx)])
      return(.wrap_angle(ci))
    }
  }
  if (parm == "A") {
...
test_phase_diff <- function(fit1, fit2, delta0 = 0,
  ...
  if (method == "difference") {
    ...
    g <- c(C2, -B2, -C1, B1)
    VU <- as.numeric(t(g) %*% Matrix::bdiag(V1, V2) %*% g)
    z <- U / sqrt(VU)
    p <- switch(alternative,
                "two.sided" = 2*pnorm(-abs(z)),
                "less" = pnorm(z),
